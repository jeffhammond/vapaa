...
void C_MPI_Type_get_name(int * type_f, char ** pname, int * resultlen, int * ierror)
{
    MPI_Datatype type = C_MPI_TYPE_F2C(*type_f);
    char * name = *pname;
    *ierror = MPI_Type_get_name(type, name, resultlen);
    C_MPI_RC_FIX(*ierror);
}

#ifdef HAVE_CFI
void CFI_MPI_Type_get_name(int * type_f, CFI_cdesc_t * name_d, int * resultlen, int * ierror)
{
    MPI_Datatype type = C_MPI_TYPE_F2C(*type_f);
