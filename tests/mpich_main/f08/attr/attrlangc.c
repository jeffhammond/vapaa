...
void ctoctest_(MPI_Fint * errs)
{
    ...
    MPI_Datatype c_integer = VAPAA_MPI_Type_fromint(VAPAA_MPI_INTEGER);
    ...
    MPI_Type_set_attr(c_integer, ctype2Key, &ctype2Attr);
    /* Test that we have the same value */
    errcnt += cmpi2readtype(c_integer, ctype2Key, &ctype2Attr, "C to C type (2)");

    /* Dup, check that the copy routine does what is expected */
    MPI_Type_dup(c_integer, &cduptype);
